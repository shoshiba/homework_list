...
def apply_filters(
    ...
    with st.sidebar:
        ...
        # 譜面難易度別フィルターの初期化
        difficulties = sorted(
            comparison["Difficulty"].unique(),
            key=lambda x: DIFFICULTY_ORDER.index(x),
        )
        if "selected_difficulty" not in st.session_state:
            st.session_state.selected_difficulty = {
                difficulty: True for difficulty in difficulties
            }
        ...
        for level in levels:
            if st.checkbox(
                level,
                value=st.session_state.selected_levels[level],
                key=f"level_{level}",
            ):
        ...
        for difficulty in difficulties:
            if st.checkbox(
                difficulty,
                value=st.session_state.selected_difficulty[difficulty],
                key=difficulty,
            ):
        ...
        for rank in ranks:
            ...
            if st.checkbox(
                rank_str,
                value=st.session_state.selected_rank[rank_str],
                key=rank_str,
            ):
